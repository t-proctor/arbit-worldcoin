...
abstract contract ERC721Soulbound is ERC721, IERC721Soulbound {
    ...
    function soul() public view virtual override returns (address) {
        return address(_soul);
    }

    function soulOwner(uint256 tokenId) public view virtual override returns (address) {
        return _soul.ownerOf(_boundTo[tokenId]);
    }

    function boundTo(uint256 tokenId) public view virtual override returns (uint256) {
        return _boundTo[tokenId];
    }

    function canReclaim(address claimant, uint256 tokenId)
        public
        view
        virtual
        override
    ...
    function _soulbind(
        ...
        require(_soul.ownerOf(soulId) == claimant, 'ERC721Soulbound: claimant not owner of soulId');
